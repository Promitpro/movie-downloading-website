...
const IMDBMovie = ({imdbTopMovie}) => {
    console.log(imdbTopMovie);
    const {title, vote_average, poster_path, id} = imdbTopMovie;
    ...
    const handleMovieClick = async () => {
        ...
        const res = await fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=29c7e473c0921537f77133fd18ffaea8&language=en-US&page=1`);
        const data = await res.json();
        console.log("Additional movie details:", data);
        navigate(`/topimdb/${id}`, { state: { movieData: data } });
        
    };
    return (
        <NavLink to={`/topimdb/${id}`} key={id}>
